...
configure_tsuga_settings() {
    ...
    if [[ -f "values.yaml" ]]; then
        ...
        # Try to extract values from values.yaml
        # Temporarily disable exit on error to allow grep to fail gracefully
        set +e
        # Look for opentelemetry-kube-stack.tsuga.otlpEndpoint pattern (nested)
        if grep -q "opentelemetry-kube-stack:" values.yaml; then
            # Extract from nested structure: opentelemetry-kube-stack.tsuga.otlpEndpoint
            current_endpoint=$(grep -A 20 "opentelemetry-kube-stack:" values.yaml | grep -A 10 "tsuga:" | grep "otlpEndpoint:" | sed -E 's/.*otlpEndpoint:[[:space:]]*["'\'']?([^"'\'']*)["'\'']?.*/\1/' | head -1)
            current_api_key=$(grep -A 20 "opentelemetry-kube-stack:" values.yaml | grep -A 10 "tsuga:" | grep "apiKey:" | sed -E 's/.*apiKey:[[:space:]]*["'\'']?([^"'\'']*)["'\'']?.*/\1/' | head -1)
        fi
        
        # If not found, try direct tsuga.otlpEndpoint at root level
        if [[ -z "$current_endpoint" ]]; then
            current_endpoint=$(grep "otlpEndpoint:" values.yaml 2>/dev/null | sed -E 's/.*otlpEndpoint:[[:space:]]*["'\'']?([^"'\'']*)["'\'']?.*/\1/' | head -1)
        fi
        
        if [[ -z "$current_api_key" ]]; then
            current_api_key=$(grep "apiKey:" values.yaml 2>/dev/null | sed -E 's/.*apiKey:[[:space:]]*["'\'']?([^"'\'']*)["'\'']?.*/\1/' | head -1)
        fi
...
deploy_helm_chart() {
    ...
    if [[ -n "$OTEL_ENDPOINT" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.tsuga.otlpEndpoint=\"$OTEL_ENDPOINT\""
    fi
    
    if [[ -n "$OTEL_API_KEY" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.tsuga.apiKey=\"$OTEL_API_KEY\""
    fi

    if [[ -n "$OTEL_ENDPOINT" && -n "$OTEL_API_KEY" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.secret.create=true"
    fi
    
    # Add collection settings if configured
    if [[ -n "$COLLECT_NETWORK" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.agent.collectNetwork=$COLLECT_NETWORK"
    fi
    
    if [[ -n "$COLLECT_PROCESSES" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.agent.collectProcesses=$COLLECT_PROCESSES"
    fi
    
    # Add cluster name if provided
    if [[ -n "$CLUSTER_NAME" ]]; then
        helm_args="$helm_args --set opentelemetry-kube-stack.clusterName=\"$CLUSTER_NAME\""
    fi
    ...
    if [[ -n "$helm_args" ]]; then
        ...
        if [[ -n "$OTEL_API_KEY" ]]; then
            masked_cmd=$(echo "$helm_cmd" | sed "s/--set opentelemetry-kube-stack\.tsuga\.apiKey=\"[^\"]*\"/--set opentelemetry-kube-stack.tsuga.apiKey=\"[MASKED]\"/")
        fi

...
| serviceAccount.name | string | "" | Name of the service account, defaulting to the release fullname. Set this with create=false to run the collectors under an account you manage. The ClusterRoleBinding targets this name either way. Leaving it empty with create=false is unsupported: the collectors then run under an account the operator names itself, which the ClusterRoleBinding does not cover, and every Kubernetes read is denied. |

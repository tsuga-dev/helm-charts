...
- **Cumulative To Delta**: Converts cumulative counters to delta. Wired into the metric pipelines only, and placed before enrichment: delta state is keyed on the full resource, so a series that starts unenriched and later gains pod metadata would look like a brand-new series and lose a datapoint.
...
- **Resource/node** (`resource/node`): Adds `k8s.node.name` from the downward API, with action `upsert`. Wired only into the node-local pipelines (`logs/node`, `metrics/node`), whose receivers all read strictly local data: `file_log` reads this node's log files, `host_metrics` this node's kernel, `kubelet_stats` this node's kubelet. It is deliberately absent from the `otlp` pipelines: OTLP arrives over a Service that load-balances across every agent, so the receiving node is not the sending pod's node. `upsert` rather than `insert` because `k8s_attributes` may have failed to enrich (cold informer cache, or an already-deleted pod), and where it did enrich the value is identical anyway — the informer only holds pods on this node.
...
- **Metrics**: `otlp`, `span_metrics` → `memory_limiter`, `cumulativetodelta`, `k8s_attributes`, `resource`, `batch` → `otlp_http/tsuga`
- **Metrics/node**: `host_metrics`, `kubelet_stats` → `memory_limiter`, `cumulativetodelta`, `k8s_attributes`, `resource`, `resource/node`, `batch` → `otlp_http/tsuga`
...
| agent.config.service.pipelines.logs | object | `{"extraExporters":[],"extraProcessors":[],"extraReceivers":[]}` | Logs pipeline configuration (otlp only). Container logs live in the logs/node pipeline, so extras set here do not reach them: repeat them under logs/node if you want them applied to container logs too. |
...
| agent.config.service.pipelines.metrics | object | `{"extraExporters":[],"extraProcessors":[],"extraReceivers":[]}` | Metrics pipeline configuration (otlp and spanmetrics only). Host and kubelet metrics live in the metrics/node pipeline, so extras set here do not reach them: repeat them under metrics/node if you want them applied to host and kubelet metrics too. |
...
| agent.config.service.pipelines.metrics.extraProcessors | list | [] | Additional processors to add to the metrics pipeline Added to default processors (memory_limiter, cumulativetodelta, k8s_attributes, resource, batch) |
| agent.config.service.pipelines.metrics.extraReceivers | list | [] | Additional receivers to add to the metrics pipeline Added to default receivers (otlp, span_metrics) |
...
| agent.config.service.pipelines.metrics/node.extraProcessors | list | [] | Additional processors to add to the metrics/node pipeline Added to default processors (memory_limiter, cumulativetodelta, k8s_attributes, resource, resource/node, batch) |
| agent.config.service.pipelines.metrics/node.extraReceivers | list | [] | Additional receivers to add to the metrics/node pipeline Added to default receivers (host_metrics, kubelet_stats) |

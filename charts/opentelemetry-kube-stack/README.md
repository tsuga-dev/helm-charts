...
| serviceAccount.name | string | "" | Name of the service account, defaulting to the release fullname. Set it together with create=false to run the collectors under an account you manage; the chart's ClusterRoleBinding targets this name either way. Leaving both create=false and this empty is the one unsupported combination: the operator then provisions an account of its own naming that the ClusterRoleBinding does not cover, so the collectors reach Ready and only then does every Kubernetes read start being denied. |
...
| targetAllocator.spec | object | {} | TargetAllocator CR spec (full passthrough) All fields are passed directly to the TargetAllocator CR spec. A spec.serviceAccount here overrides the account the chart would otherwise set, and is emitted in place of it rather than alongside it. Ref: https://github.com/open-telemetry/opentelemetry-operator/blob/main/docs/api.md#targetallocator |

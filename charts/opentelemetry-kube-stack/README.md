...
| cluster.collectk8sevents | bool | true | Collect Kubernetes Warning events as logs  Events are the only source for OOMKilled, FailedScheduling, Evicted, ErrImagePull, FailedMount and failing probes — no metric receiver reports them. RBAC for events is already granted, so this costs no extra configuration.  Warning events only, filtered by field_selector on the API server, so Normal events are never watched or transferred. Normal events are the high-volume, low-value majority and restate what the pods object stream already carries. |
...
| deriveServiceIdentity | bool | true | Derive service identity from Kubernetes When true, the k8s_attributes processor computes service.name, service.version and service.namespace for telemetry that does not already carry them, following the semantic conventions' Kubernetes attribute guidance.  This matters most for metrics and traces. Tsuga already fills a missing service name and version for logs during ingest, but that postprocessing is documented for logs only, so metrics and spans from a workload that never set a service name arrive with none.  Nothing already on the telemetry is overwritten, so an app that sets its own service.name keeps it.  One caveat worth knowing before enabling this. Of the two ways this chart lets a pod declare a service name, only the ANNOTATION form survives: the processor applies label rules, then copies the well-known app.kubernetes.io/name and /instance labels over the top, then applies annotation rules. So a resource.opentelemetry.io/service.name LABEL is overwritten by a well-known label, while the annotation of the same name wins. Use the annotation form, or leave this off.  The derivation reads the well-known app.kubernetes.io/name, app.kubernetes.io/instance and app.kubernetes.io/version labels. The semantic conventions ask tools to make that opt-in; this flag ships on instead, because the gap it closes is silent. Set it to false to stop reading those labels. |
...
| resourceDetection.detectors | list | ["env"] | Detectors to run, in order  Where two detectors can supply the same attribute, the FIRST one listed wins. Existing attributes on the telemetry are never replaced (override: false).  env is the only default: it reads OTEL_RESOURCE_ATTRIBUTES and returns nothing, without error, when that is unset.  Recipes, with the caveats that matter:   AWS/EKS:  [env, eks, ec2]     eks on its own only supplies cloud.provider and cloud.platform; every     other EKS attribute is default-off. ec2 is what supplies cloud.region,     cloud.account.id, cloud.availability_zone, host.id and host.name, and     must come after eks so cloud.platform stays aws_eks.     eks is NOT safe on a non-EKS cluster: KUBERNETES_SERVICE_HOST is set on     every Kubernetes cluster, so its check falls through to the Kubernetes     API and can error, which prevents startup.   GCP/GKE:  [env, gcp]     gcp supplies cloud.provider, cloud.region, cloud.account.id, host.id,     host.name and k8s.cluster.name. Returns empty without error off GCE.   Azure/AKS: [env, aks, azure]     azure supplies region, account id and host id; aks adds cloud.platform.     Both return empty without error outside Azure.  k8snode is deliberately absent. It reads the node object from the Kubernetes API and fails Start() if that call fails, and the only attribute it adds that this chart does not already set is k8s.node.uid. |
| resourceDetection.enabled | bool | false | Enable the resourcedetection processor  Off by default, and opt-in on purpose. A detector that fails takes the collector down with it: the processor's Start() propagates the error and stops the collector from starting. There is no escape hatch on any version this chart can target — the processor.resourcedetection.propagateerrors gate had already gone stable, and therefore become permanently on, before this chart's floor, and was deleted outright in v0.150.0. So this is not a safe thing to switch on for every cluster. |
| resourceDetection.timeout | string | "15s" | Per-detector timeout 15s, the value the processor README uses for EKS, rather than the 5s factory default: the retry uses exponential backoff with no maximum elapsed time, so a failing detector spends the whole budget before giving up. |

...
### Added
- An opt-in eBPF profiling collector, `profiling.enabled`. A privileged DaemonSet running the OpenTelemetry eBPF profiler distro, which samples CPU profiles from every process on the node with no application changes — no SDK, no re-instrumentation, no restart of the profiled workloads — and exports them to Tsuga as `profile.<sample_type>.<service.name>` metrics. It is a separate collector rather than a pipeline on the agent because the `profiling` receiver ships only in that distro, and it needs `privileged` plus `hostPID`, which the agent does not. Requires profiles ingestion to be enabled on the target Tsuga cluster, and a release namespace that admits privileged pods
...
- `profiling.k8sAttributesMetadata` and `profiling.dropAttributes`, both deliberately narrow. Tsuga carries the whole stack as one `stack_trace` dimension and turns every other resource attribute into a dimension on top of it, so the collector drops what the receiver sets per process (`process.pid`, `process.executable.path`) and the pod-scoped attributes `k8s_attributes` resolved. `container.id` is dropped after `k8s_attributes` rather than never collected: it is the only key the receiver gives the processor to associate a pod on
...
- `resourceDetection` is not wired into this collector whatever `resourceDetection.enabled` says: `host.id` is one value per node, which would multiply the whole stack space by the node count
- `profiling.samplesPerSecond` defaults to 20 and should stay there. The rate is not carried on the exported metric, so it cannot be recovered at read time, and Tsuga converts samples into CPU time assuming 20 Hz
- The receiver also attaches `thread.id`, `thread.name` and `cpu.logical_number` at sample level. Those become dimensions too, and nothing in this distro can strip sample attributes
...
### Added
- redaction.enabled, which masks credentials in the agent's logs, metrics and traces pipelines via the collector redaction processor, off by default
- redaction.config, passed to the collector as-is, defaulting to the credential rule set from the Tsuga redaction guide
